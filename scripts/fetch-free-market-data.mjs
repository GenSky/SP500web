...
const minimumRows = constituents.length <= 10 ? constituents.length : Math.ceil(constituents.length * 0.9);
if (rows.length < minimumRows) {
  throw new Error(`Refresh returned only ${rows.length}/${constituents.length} rows; refusing to overwrite the last good dataset.`);
}
